...
class CustomSerialNo(Document): # Or inherit from ERPNext's SerialNo if extending directly
    ...
    # --- LLP Logic ---
    def update_life_data(self, hours_added=0, cycles_added=0, transaction_date=None, aircraft=None, engine=None, work_order=None, movement_type="Usage"):
        """
        Call this method from Stock Entry or Work Order completion
        to update the life used on an LLP.
        """
        if not frappe.db.get_value("Item", self.item_code, "is_tracked_component"):
        ...
        log_entry.engine = engine # Link to Engine DocType if exists
